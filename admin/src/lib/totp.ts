...
export async function getTotpSecret(): Promise<string | null> {
  const snap = await adminDb.collection('admin').doc('totp').get()
  if (!snap.exists) return null
  return snap.data()?.secret ?? null
}

export async function createTotpSecret(): Promise<string> {
  const secret = generateTotpSecret()
  await adminDb.collection('admin').doc('totp').set({ secret, setupAt: new Date().toISOString() })
  return secret
}
